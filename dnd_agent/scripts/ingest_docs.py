...
def read_pdf_file_with_pages(path: Path) -> list[tuple[int, str]]:
    """Return a list of (page_number, text) tuples, one per PDF page."""
    try:
        import pypdf
    except ImportError:
        print(f"  Skipping {path.name}: install 'pypdf' to process PDFs (uv add pypdf)")
...
def ingest(docs_dir: str, chunk_size: int, chunk_overlap: int) -> None:
    ...
    for file_path in sorted(files):
        ...
        if file_path.suffix.lower() == ".pdf":
            # Process PDFs page-by-page so we can record the source page number
            pages = read_pdf_file_with_pages(file_path)
            if not pages:
                continue
